...
何かをスナップショットする前に、機械的に安全な違反は先に直しておきましょう —
オートフィックスで無料で直せたはずのものをベースラインに凍結する理由はありません。
...
`--fix` はファイルをその場で書き換えます。終了コードは修復した件数ではなく
「まだ直っていない件数」を反映するため、ここで非ゼロ終了になるのは正常です。
現時点でオートフィックス対応なのは 8 ルール中 3 ルール
（`madr/status-enum`、`madr/date-iso8601`、`madr/supersedes-bidirectional`）
だけなので、これはあくまで一部の削減にすぎません — 残りは次のステップのベースラインが
吸収します。
...
（まだであればステップ 2 の修正も一緒に）コミットしてください。**
...
ステップ 4 を CI に組み込めば、レガシーな負債はあなたのペースで返済しつつ、初日から
