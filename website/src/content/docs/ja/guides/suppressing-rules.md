...
- **MADR v2 のメタデータリストの項目の間に抑制コメントを置くと、リストが
  分断されます。** v2 のメタデータは Markdown のリストです（`* Status:
  accepted` / `* Date: 2024-1-5`）。項目の**間**に HTML コメントを挿入すると
  リストが 2 つに分断され、v2 メタデータブリッジはその分断点までしか
  読み取らないため、コメント以降のフィールドはパース済みメタデータから
  静かに消え去ります。その結果、行単位で抑制できていたはずの
  `invalidDate` が、抑制できないファイルレベルの `missingDate`
  （上記「行番号を持たない診断」を参照）に悪化してしまいます —
  抑制する前より悪い、抑制不能なエラーになるということです。ディレクティブは
  リスト全体の上に置くか、v2 ファイルには `madr-lint-disable-file` を
  使ってください。
  [#73](https://github.com/knktkc/madr-lint/issues/73) で追跡されています。
