...
- **MADR v2 のメタデータリストの項目の間にディレクティブを置けます。** v2 の
  ...
  して読み取ります。`disable-next-line` はコメントの下のフィールドを通常どおり
  対象にでき、そのフィールドは行番号を保持したまま自動修正の対象にもなります。
  メタデータブロックを終わらせるのは、段落・コードブロック・水平線・引用・
  任意の深さの見出し、そして `<div>` や `<details>` のような表示される HTML
  です。
