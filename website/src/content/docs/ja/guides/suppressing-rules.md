...
- **ディレクティブは複数行に分けて書けます。** `<!--` … `-->` を複数行に分けて
  書いた場合、`disable-next-line` は `-->` の次の空行でない行を対象にします。
  1 行で書いたときと同じ行です。
  ...
  * Date: 2024-1-5
  ...
  ただし、キーワードとルールのリストは 1 つのコメントの中に収めてください。
  2 つのコメントに分けて書くと 1 つのディレクティブにはなりません。`-->` も
  単独の行に置いてください。同じ行に続けて何かを書くと（`--> * Date: 2024-1-5`）
  その内容までコメントに取り込まれ、`-->` で終わらなくなるため、ディレクティブ
  として認識されません。警告もないまま、何も抑制しなくなります。
